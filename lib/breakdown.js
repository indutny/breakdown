...
class Breakdown {
  ...
  _onInit(id, type, triggerId, resource) {
    let executionId = async_hooks.executionAsyncId();
    if (executionId === 0) {
      executionId = triggerId;
    }

    let parents = null;
    if (this.parentsById.has(executionId)) {
      parents = this.parentsById.get(executionId);
    }
    parents = [ executionId, parents ];
    this.parentsById.set(id, parents);
  ...
  _onAfter(id) {
    ...
    spin = Number(spin) * 1e-9;

    const event = this._findParent(this.parentsById.get(id) || null);
    if (!event) {
      return;
    }
    event.selfSpin += spin;
