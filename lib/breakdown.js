...
class Breakdown {
  ...
  _onIncomingRequest(event, parser, incoming) {
    ...
    this._startEvent(event, 'HTTP_SERVER_REQUEST', {
      ...
      headers: this.options.sanitize(incoming.headers),
      url: this.options.sanitize(incoming.url),
    });
  }
  ...
  _onClientRequest(event, outgoing) {
    ...
    this._startEvent(event, 'HTTP_CLIENT_REQUEST', {
      ...
      path: this.options.sanitize(outgoing.path),
      headers: this.options.sanitize(outgoing.getHeaders()),
    });
  ...
  _onGetAddrInfo(event, resource) {
    ...
    resource.callback = function(error, address) {
      ...
      breakdown._logEvent(event, {
        error: error ? breakdown.options.sanitize(error.message) : false,
        address: breakdown.options.sanitize(address),
      });
    ...
    this._startEvent(event, 'DNS_LOOKUP', {
      family,
      hostname: this.options.sanitize(resource.hostname),
    });
  }
